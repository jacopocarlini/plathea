...
public class MainSystem {
    ...
    public synchronized static  ReturnRoomMessage deleteRoom(int roomId){
        if(rooms.containsKey(roomId)){          
            rooms.get(roomId).interfaceJNI.myProcess.destroy();
            rooms.remove(roomId);
    ...
    TrackedPerson[] tp = rooms.get(roomID).interfaceJNI.getTrackedPeople();
        ...
        for(TrackedPerson elem : tp){
            System.out.println("(float)nameID="+elem.nameID);
            System.out.println("(float)X="+elem.X);
            System.out.println("(float)Y="+elem.Y);
            System.out.println("(float)ID="+elem.ID);
