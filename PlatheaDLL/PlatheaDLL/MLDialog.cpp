...
static void PrepareComponentValuesListView(HWND listViewControl, CvMat *componentValues) {
	...
	if (componentValues) {
		...
		lc.cx = 30;
		char text[] = "#";
		mbstowcs(lc.pszText, text, strlen(text) + 1);//Plus null
		lc.iSubItem = 0;
...
static void PrepareListViewControl(HWND listViewControl) {
	...
	lc.cx = 80;
	char text1[] = "Position";
	mbstowcs(lc.pszText, text1, strlen(text1) + 1);//Plus null
	lc.iSubItem = 1;
	ListView_InsertColumn(listViewControl, 1, &lc);

	lc.cx = 80;
	char text2[] = "Angle";
	mbstowcs(lc.pszText, text2, strlen(text2) + 1);//Plus null
	lc.iSubItem = 2;
	ListView_InsertColumn(listViewControl, 2, &lc);

	lc.cx = 30; 
	char text3[] = "Classification";
	mbstowcs(lc.pszText, text3, strlen(text3) + 1);//Plus null
	lc.iSubItem = 3;
