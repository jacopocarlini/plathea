...
bool StereoCalibration::SaveToFolder(char *folderName, int mask) {
...
bool StereoCalibration::LoadFromFolder(char *folderName, int mask) {
	DBOUT(folderName);
	DBOUT(mask);
	DBOUT(LEFT_INTERNAL);
...
bool ExternalCalibration::LoadFromFolder(char *folderName) {
	CvMat *rm, *tv;
	int maxLen = int(strlen(folderName)) + 255;
	char * tempStr = new char[maxLen];
