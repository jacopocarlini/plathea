					...
					struct feature*** nbrs, int max_nn_chks )
{
	...
	while( min_pq->n > 0  &&  t < max_nn_chks )
	{
		...
		for( i = 0; i < expl->n; i++ )
		{
			...
			bbf_data_p = (bbf_data *) malloc( sizeof( struct bbf_data ) );
			if( ! bbf_data_p )
			{
			...
			bbf_data_p->old_data = tree_feat->feature_data;
			bbf_data_p->d = descr_dist_sq(feat, tree_feat);
			tree_feat->feature_data = bbf_data_p;
	...
	for( i = 0; i < n; i++ )
	{
		bbf_data_p = (bbf_data *) _nbrs[i]->feature_data;
		_nbrs[i]->feature_data = bbf_data_p->old_data;
		free( bbf_data_p );
	}
	*nbrs = _nbrs;
	return n;
...
{
	struct kd_node* kd_node_p;

	kd_node_p = (kd_node *) malloc( sizeof( struct kd_node ) );
	memset( kd_node_p, 0, sizeof( struct kd_node ) );
	kd_node_p->ki = -1;
	kd_node_p->features = features;
	kd_node_p->n = n;
