...
//void GrabberCallback(void * pBits, int width, int height, int size) {
	...
	if (waitResult == WAIT_OBJECT_0 && faces && faces->total) {
		...
		cvResize(colorImage, preSizedImage, CV_INTER_CUBIC);
		cvResetImageROI(colorImage);
		int faceIndex = underConstructionSubject.AddTrainingFace(TrainingFace(preSizedImage));
		...
		case WM_INITDIALOG:
			{
				...
				/*cvSetCaptureProperty(cameraCapture, CV_CAP_FFMPEG, true);
				cvSetCaptureProperty(cameraCapture, CV_CAP_PROP_FPS, 30);
				cvSetCaptureProperty(cameraCapture, CV_CAP_PROP_FRAME_WIDTH, 1024);*/
				SetTimer(hwndDlg, 1, 100, GrabberCallback);
				/*GrabberOpen(NULL, GrabberCallback);
				GrabberStart();*/
